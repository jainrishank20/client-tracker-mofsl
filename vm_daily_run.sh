#!/bin/bash
# Daily trade pipeline — triggers GitHub Actions from VM (Indian IP).
# GHA handles the actual download, import, GSheet sync, and Telegram notification.
#
...
_tg_notify() {
  ...
  if [ -n "$TG_TOKEN" ] && [ -n "$TG_CHAT" ]; then
    ...
    if [ "$LAST" -gt 0 ]; then
      MSG="Pipeline triggered ($(TZ='Asia/Kolkata' date '+%d %b %I:%M %p IST')) — GHA running. Check /vmlog in ~10 mins."
    else
      ERRMSG=$(tail -5 "$LOG" | grep -i "error\|fail" | tail -1)
...
# Trigger GHA: full pipeline (download + import + GSheet sync + Telegram)
HTTP=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
  -H "Authorization: token ${GITHUB_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "{\"ref\":\"main\",\"inputs\":{\"skip_download\":\"false\",\"full_history\":\"${IS_FULL}\"}}" \
